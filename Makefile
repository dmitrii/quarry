...
# Pin the commit that --version reports. Outside a git checkout there is
# nothing to read, so the stamp is skipped and --version prints just the
# version number.
stamp:
	@if git -C "$(CURDIR)" rev-parse HEAD >/dev/null 2>&1; then \
	  ...
	  echo "stamp: pinned $$(git -C "$(CURDIR)" log -1 --format='%h (%cs)')"; \
	else \
